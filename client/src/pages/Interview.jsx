...
const API = "https://interviewai-bnux.onrender.com"
const TOTAL_Q = 5
...
function Interview() {
  ...
  const parseScores = (text) => {
    ...
    const overall =
      getScore(/Overall\s*Score:\s*(\d+(\.\d+)?)/i) ||
      getScore(/Overall:\s*(\d+(\.\d+)?)/i) ||
      0
    const technical =
      getScore(/Technical\s*Knowledge:\s*(\d+(\.\d+)?)/i) ||
      getScore(/Technical:\s*(\d+(\.\d+)?)/i) ||
      overall
    ...
    return {
      ...
      communication,
      confidence,
      tip: tip || "Keep practicing to see detailed advice."
    }
  }
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      try {
        const res = await axios.get(`${API}/api/interview-tracks`)
        setTracks(res.data || [])
  ...
  const handleStartSession = async () => {
    if (!file || !selectedTrack || starting) return
    try {
      setStarting(true)
      setLoading(true)
      setAiTyping(true)
      ...
      const questions = (res.data.questions || [res.data.question]).slice(0, TOTAL_Q)
      ...
      alert(err.response?.data?.error || "Failed to start mock interview. Please try again.")
    } finally {
      setAiTyping(false)
      setLoading(false)
      setStarting(false)
    ...
    try {
      ...
      if (typeof res.data.score === "number") {
        setSessionScores((prev) => {
          const next = [...prev]
          next[currentIndex] = res.data.score
          return next
        })
      }

      if (res.data.finished || currentIndex >= TOTAL_Q - 1) {
        setFinished(true)
      }
    } catch (err) {
  ...
  const handleNextQuestion = () => {
    ...
    if (nextIdx >= allQuestions.length || nextIdx >= TOTAL_Q) {
      setFinished(true)
      return
    }
    ...
    <div className="space-y-8 max-w-4xl mx-auto animate-fade-in pb-12">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          Simulate a real technical screening. Each session has exactly {TOTAL_Q} questions.
        </p>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-4 flex items-center justify-between border-gray-900/60 max-w-md mx-auto">
        {[["1", "Setup"], ["2", "Session"], ["3", "Feedback"]].map(([num, label], i) => (
          <div key={num} className="flex items-center gap-2">
            {i > 0 && <div className="w-10 h-px bg-gray-800 mr-2" />}
            <div
              className={`w-8 h-8 rounded-xl font-bold flex items-center justify-center text-xs border ${
            ...
            <span className={`text-xs font-semibold ${currentStep >= parseInt(num) ? "text-gray-200" : "text-gray-500"}`}>
              {label}
            </span>
          </div>
        ))}
      ...
      {currentStep === 1 && (
        <div className="glass-panel rounded-3xl p-8 shadow-xl space-y-8 animate-slide-up">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-3xl">
              {tracks.length > 0 ? (
                ...
                <div className="rounded-2xl border border-gray-800 bg-gray-950 p-4 text-sm text-gray-500">
                  Loading tracks...
                </div>
              )}
            </div>
          ...
          {selectedTrack && (
            <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-5 grid sm:grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Interview Type</p>
                <p className="text-sm font-bold text-white mt-0.5">{selectedTrack.label}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Difficulty</p>
                <p className="text-sm font-bold text-white mt-0.5">{difficulty}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Questions</p>
                <p className="text-sm font-bold text-white mt-0.5">{TOTAL_Q} Questions</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Estimated Duration</p>
                <p className="text-sm font-bold text-white mt-0.5">10–15 minutes</p>
              </div>
            </div>
          )}
          ...
          <button
            ...
            disabled={!file || !selectedTrack || loading || starting}
      ...
      {loading && aiTyping && (
        <div className="glass-panel rounded-3xl p-10 text-center space-y-6 border-dashed border-indigo-500/30">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">Preparing your session</h3>
            <p className="text-xs text-gray-500 mt-2 max-w-sm mx-auto leading-relaxed">
              Loading {TOTAL_Q} {difficulty}-difficulty questions for {selectedTrack?.label || "your track"}…
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6 animate-slide-up">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400 font-semibold">
              ...
              Question {currentIndex + 1} of {TOTAL_Q}
              <span className="text-gray-600">·</span>
              {Math.round((currentIndex / TOTAL_Q) * 100)}% complete
            </div>
          ...
          <div className="h-1.5 rounded-full bg-gray-900 overflow-hidden">
            <div
              ...
              style={{ width: `${(currentIndex / TOTAL_Q) * 100}%` }}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              placeholder="Provide a clear, structured technical answer — explain trade-offs, approach, and examples where relevant..."
              value={answer}
              disabled={loading}
      ...
      {currentStep === 3 && (
        <div className="space-y-8 animate-slide-up">
          {finished && scores && (
            <div className="glass-panel rounded-3xl p-8 border-indigo-500/20 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-indigo-400 font-semibold mb-1">
                    Session completed · {TOTAL_Q}/{TOTAL_Q} questions
                  </p>
                  <h3 className="text-2xl font-extrabold text-white">Interview Complete</h3>
                  {avgSession && (
                    <p className="text-sm text-gray-400 mt-1">
                      Session average score: <span className="text-white font-bold">{avgSession}/10</span>
                    </p>
                  )}
                </div>
                <button
                  type="button"
                  onClick={handleReset}
                  className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition text-sm"
                >
          ...
          {finished && !feedback && (
            <div className="glass-panel rounded-3xl p-10 text-center space-y-4 border-indigo-500/20">
              ...
              <h3 className="text-2xl font-extrabold text-white">Interview Complete!</h3>
              <p className="text-gray-400 text-sm">You answered all {TOTAL_Q} questions.</p>
              <button
                type="button"
                onClick={handleReset}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-extrabold text-xl text-white">
                        {finished ? "Final Assessment" : "Answer Evaluated"}
                      </h3>
                      <p className="text-xs text-gray-500 mt-1">
                        Question {currentIndex + 1} of {TOTAL_Q} · Score: {scores.overall}/10
                      </p>
                    </div>
                  </div>
                ...
                {[
                  ...
                  <div key={label} className="glass-panel rounded-2xl p-5 border border-gray-900 space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-extrabold text-indigo-400">{value}/10</span>
                    </div>
